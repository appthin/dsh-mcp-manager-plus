 ...
 */
function buildInventory(ctx, document) {
  ...
  for (const row of document.rows) {
    ...
    if (serverName === '') {
      issues.push(`行 ${row.id || '(无 id)'} 缺少 config.serverName，已跳过管理`);
      continue;
    }
...
function validateServerName(name) {
  if (typeof name !== 'string' || name.trim() === '') return '服务名不能为空';
  if (!/^[A-Za-z0-9_-]{1,32}$/u.test(name)) {
    return '服务名只能包含字母、数字、下划线和连字符，长度 1–32';
  }
  return null;
}
 ...
 */
function commit(patchPath, text, yaml, schema) {
  const invalid = validateDocument(text, yaml, schema);
  if (invalid !== null) throw new Error(`${invalid}（已取消写入）`);
 ...
 */
async function saveServer(ctx, input) {
  const serverName = typeof input.serverName === 'string' ? input.serverName.trim() : '';
  const invalid = validateServerName(serverName);
  ...
  return queuedWrite(async () => {
    ...
    if (editing && existing === undefined) {
      throw new Error(`未找到要编辑的服务器 ${originalName}`);
    }
    if (!editing && document.rows.some((row) => row.config.serverName === serverName)) {
      throw new Error(`服务名 ${serverName} 已存在`);
    }
    ...
    if (existing !== undefined) {
      ...
      commit(patchPath, text, yaml, schema);
    ...
    if (taken.has(id)) throw new Error(`行 id ${id} 已被占用，请换一个服务名`);
 ...
 */
async function toggleServer(ctx, input) {
  const id = typeof input.id === 'string' ? input.id : '';
  if (id === '') throw new Error('缺少行 id');
  ...
  return queuedWrite(async () => {
    ...
    if (!ownedByPlugin && id === '') throw new Error('内置行缺少 id，无法切换');
 ...
 * @param input - `{ id }`.
 */
async function removeServer(ctx, input) {
  const id = typeof input.id === 'string' ? input.id : '';
  if (id === '') throw new Error('缺少行 id');
  const { patchPath } = resolvePatchPath(ctx);
  const { yaml, schema } = await loadYaml(ctx);
  ...
  return queuedWrite(async () => {
    ...
    if (target === undefined) {
      throw new Error(`未找到受管行 ${id}（部署内置的服务器不能删除，只能停用）`);
    }
 ...
 */
async function restartServer(ctx, input) {
  const id = typeof input.id === 'string' ? input.id : '';
  if (id === '') throw new Error('缺少行 id');
  const { patchPath } = resolvePatchPath(ctx);
  const { yaml, schema } = await loadYaml(ctx);
  const document = readDocument(readPatchText(patchPath), yaml, schema);
  const inventory = buildInventory(ctx, document);
  const server = inventory.servers.find((entry) => entry.id === id);
  if (server === undefined) throw new Error(`未找到服务器 ${id}`);
  if (!server.enabled) throw new Error(`服务器 ${server.serverName} 已停用，请先启用`);
  const entry = findEntry(ctx, server.serverName);
  if (entry === undefined) throw new Error(`服务器 ${server.serverName} 未挂载，无法重启`);
  if (typeof entry._dispose !== 'function' || typeof entry.init !== 'function') {
    throw new Error('当前载入器不支持就地重启，请重启 dsh 进程');
  }
  await entry._dispose();
  await entry.init();
  return `已重启 ${server.serverName}`;
}
 ...
 */
async function importServers(ctx, input) {
  const { servers: parsed, source } = parseMcpConfig(input.json);

  const originalName = typeof input.originalName === 'string' ? input.originalName.trim() : '';
  if (originalName !== '') return importSingleEdit(ctx, parsed, originalName, source);
  ...
  if (created.length === 0 && updated.length === 0) {
    throw new Error(`未能导入任何服务器：${skipped.join('；')}`);
  }

  const parts = [`已识别 ${source} 配置`];
  if (created.length > 0) parts.push(`新增 ${created.join('、')}`);
  if (updated.length > 0) parts.push(`更新 ${updated.join('、')}`);
  if (skipped.length > 0) parts.push(`跳过 ${skipped.join('；')}`);
  return { message: parts.join('；'), created, updated, skipped };
}
 ...
 * @returns `{ message, created, updated }`.
 */
async function importSingleEdit(ctx, parsed, originalName, source) {
  if (parsed.length === 0) throw new Error('未在配置中找到任何 MCP 服务器条目');
  if (parsed.length > 1) {
    const names = parsed.map((entry) => entry.serverName).join('、');
    throw new Error(`编辑模式一次只能保存一个服务器，但配置里有 ${parsed.length} 个：${names}`);
  }
  ...
  const renamed = serverName !== originalName ? `（由 ${originalName} 重命名）` : '';
  return {
    message: `已识别 ${source} 配置；更新 ${serverName}${renamed}`,
    created: [],
    updated: [serverName],
    skipped: [],
  };
  ...
  const handler = async (req, res) => {
    ...
    try {
      if (route === '/servers' && req.method === 'GET') {
        sendJson(res, 200, await listServers());
        return;
      }
      ...
      if (route === '/save' && req.method === 'POST') {
        const result = await saveServer(ctx, await readJsonBody(req));
        cache = null;
        sendJson(res, 200, { ok: true, ...result, state: await listServers() });
        return;
      }
      if (route === '/toggle' && req.method === 'POST') {
        const result = await toggleServer(ctx, await readJsonBody(req));
        cache = null;
        sendJson(res, 200, { ok: true, ...result, state: await listServers() });
        return;
      }
      if (route === '/remove' && req.method === 'POST') {
        const result = await removeServer(ctx, await readJsonBody(req));
        cache = null;
        sendJson(res, 200, { ok: true, ...result, state: await listServers() });
        return;
      }
      if (route === '/restart' && req.method === 'POST') {
        const message = await restartServer(ctx, await readJsonBody(req));
        cache = null;
        sendJson(res, 200, { ok: true, message, state: await listServers() });
        return;
      }
      if (route === '/import' && req.method === 'POST') {
        const result = await importServers(ctx, await readJsonBody(req));
        cache = null;
        sendJson(res, 200, { ok: true, ...result, state: await listServers() });
        return;
      }
