...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * The active locale. Read through the locale service when it is composed,
     * so the page follows the UI language; a page that cannot reach locale
     * still renders, defaulting to Chinese — this deployment's default.
     */
    var localeRef = { current: 'zh' };

    /** The settings page itself. */
    function McpManagerPage() {
      var t = makeT(localeRef.current);
      ...
      // Swap this page's settings nav-row icon. The shell renders the nav list
      // before mounting a section, so the row is already in the DOM by the
      // time this runs.
      useEffect(function () {
    ...
    exports.apply = function apply(ctx) {
      ...
      if (locale !== undefined && typeof locale.register === 'function') {
        ...
        // Track the active locale so the page follows the UI language without
        // re-registering the section on every change.
        try {
          var read = function () {
            localeRef.current = locale.getLocale().id === 'en' ? 'en' : 'zh';
          };
          read();
          ctx.effect(function () {
